...
def download_extended_page(url, chromedriver, browser):
            ...
            XHR.prototype.send = function(data) {
                ...
                function stateChanged() {
                    if (readyState == 4) {
                        ...
                        XHR.prototype._data.push(response);
                    }
                }
    ...
    return images, get_all_tabs(source)
...
def build_search_url(search_term, params, url, similar_images, specific_site, safe_search):
    # check safe_search
    ...
    if url:
        ...
        keywordem = similar_images(similar_images)
...
def _get_all_items(image_objects, main_directory, dir_name, limit, arguments):
    ...
    while count < limit + 1 and i < len(image_objects):
        ...
        # to be contrary to the implementation details. 
            ...
            object = format_object(image_objects[i])
            ...
            if download_status == "success":
                ...
                if arguments['thumbnail'] or arguments["thumbnail_only"]:
                    download_status, download_message_thumbnail = download_image_thumbnail(
...
def download(arguments):
    ...
    if __name__ != "__main__":
        ...
        if 'config_file' in arguments:
            ...
            total_errors = 0
            for rec in records:
                paths, errors = download_executor(rec)
                for i in paths:
                    paths_agg[i] = paths[i]
                if not arguments["silent_mode"]:
        ...
        # if the calling file contains params directly
            ...
            paths, errors = download_executor(arguments)
            for i in paths:
                paths_agg[i] = paths[i]
            if not arguments["silent_mode"]:
                if arguments['print_paths']:
...
def download_executor(arguments):
    ...
    if arguments['keywords_from_file']:
        search_keyword = keywords_from_file(arguments['keywords_from_file'])
        ...
        for sky in suffix_keywords:  # 2.for every suffix keywords
            ...
            while i < len(search_keyword):  # 3.for every main keyword
                ...
                if not arguments["no_download"]:
                    create_directories(main_directory, dir_name, arguments['thumbnail'],
                ...
                url = build_search_url(search_term, params, arguments['url'], arguments['similar_images'],
                                            arguments['specific_site'],
                                            arguments['safe_search'])  # building main search url

                if limit < 101:
                    images, tabs = download_page(url)  # download page
                else:
                    images, tabs = download_extended_page(url, arguments['chromedriver'], arguments['browser'])
                ...
                items, errorCount, abs_path = _get_all_items(images, main_directory, dir_name, limit,
                ...
                if arguments['related_images']:
                    ...
                    for key, value in tabs.items():
                        ...
                        if limit < 101:
                            images, _ = download_page(value)  # download page
                        else:
                            images, _ = download_extended_page(value, arguments['chromedriver'], arguments['browser'])
                        create_directories(main_directory, final_search_term, arguments['thumbnail'],
                                                arguments['thumbnail_only'])
                        _get_all_items(images, main_directory, search_term + " - " + key, limit, arguments)
...
def main():
    ...
    for arguments in records:
        ...
        if arguments['single_image']:  # Download Single Image using a URL
            single_image(arguments['single_image'])
        else:  # or download multiple images based on keywords/keyphrase search
            paths, errors = download(arguments)  # wrapping response in a variable just for consistency
            total_errors = total_errors + errors
